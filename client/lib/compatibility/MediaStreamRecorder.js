...
MediaStreamRecorder = function(mediaStream) {
    ...
    this.start = function(timeSlice) {
        ...
        if(!Recorder) console.log('window.StereoRecorder does not exist');

        // video recorder (in WebM format)
        if (this.mimeType.indexOf('video') != -1) {
            Recorder = IsChrome ? window.WhammyRecorder : window.MediaRecorderWrapper;
            if(!Recorder) console.log('window.WhammyRecorder does not exist');
        }
...
StereoRecorder = function(mediaStream) {
    // void start(optional long timeSlice)
    // timestamp to fire "ondataavailable"
    this.start = function(timeSlice) {
...
MultiStreamRecorder = function(mediaStream) {
    if (!mediaStream) throw 'MediaStream is mandatory.';
